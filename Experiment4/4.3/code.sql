...
select c.customer_name, o.* from customers c right join orders o on c.customer_id=o.customer_id;
select p.product_name, c.category_name from products p full outer join categories c on p.category_id=c.category_id;
select  c.category_name, p.product_name, p.price from products p right join categories c on c.category_id=p.category_id;
